...
Nexiant Voice currently caps each recording at 30 seconds. When that limit is reached, transcription
...
👍 [this issue](https://nexiant.ai/issues/18) to help prioritize it.

### Only the default microphone is supported

Nexiant Voice records from your system's default microphone. There currently is no in-app microphone
...
**Why this happens:** Nexiant Voice relies on the [XDG Remote Desktop Portal](https://flatpak.github.io/xdg-desktop-portal/docs/doc-org.freedesktop.portal.RemoteDesktop.html) standard
...
If that would be useful to you, [let us know](https://nexiant.ai/issues/19).
...
**Why this happens:** Nexiant Voice sends keystrokes through the XDG Remote Desktop Portal.
...
**Why this happens:** After transcription, Nexiant Voice minimizes or hides its main window,
...
Nexiant Voice window rather than your target application.
...
**Why this happens:** Some applications cannot process keystrokes as fast as Nexiant Voice sends them,
...
### Nexiant Voice appears on the wrong workspace and doesn't type into my active application

**Symptom:** On a multi-workspace setup, the main Nexiant Voice window opens on a different workspace,
...
By default, Nexiant Voice minimizes the main window before typing.
...
**Why this happens:** Nexiant Voice uses the
...
only available on newer desktop environments, so Nexiant Voice only shows the **Configure** button when it
...
2. **Reset the shortcut from the command line.** Follow the
   [testing instructions](https://nexiant.ai/stargate/blob/main/docs/TESTING.md) from the Stargate
   project, replacing the application ID with `io.voicestream.SpeedOfSound`. This will delete the existing
   shortcut binding. Restart Nexiant Voice afterward, the **Set Up** button will reappear, and you can
   configure a new shortcut.

### I don't want the Nexiant Voice main window to show every time I dictate.

**Symptom:** The main window appears on every dictation, which feels intrusive for frequent voice typing.

**Why this happens:** By default, Nexiant Voice presents its main window each time you trigger dictation
...
**Note:** Because Nexiant Voice uses the Desktop Portals standard, your desktop environment will still
