...
By default, `Super+Z` starts and stops listening, but only when the Nexiant Voice window is open and focused.
For a better experience, we recommend setting up a global shortcut in Preferences. 
This lets you keep the window minimized or hidden and trigger Nexiant Voice from anywhere, typing directly into any app.
...
1. Open **Preferences** in Nexiant Voice and click the **Set Up** button under **Global Shortcut**.
...
2. A system dialog will appear prompting you to assign a keyboard shortcut.
   By default, Nexiant Voice suggests `Super+Z`, but you can enter any key combination(s) you prefer.
...
3. *(If prompted)* An additional dialog may ask you to allow the application to inhibit
   shortcuts. This means Nexiant Voice will be able to capture your chosen key combination system-wide
...
You can now trigger Nexiant Voice from anywhere.
...
[latest release page](https://nexiant.ai/releases/latest).
...
4. Click the **+** button to add a new shortcut and fill in the fields:
    - **Name:** anything you like, e.g. `Nexiant Voice`
    - **Command:** the full path to the trigger script, e.g. `/home/your-username/voicestream/trigger.sh`
...
- **Bus:** Session bus
- **Destination:** `io.voicestream.SpeedOfSound`
- **Object path:** `/io/voicestream/SpeedOfSound`
...
gdbus call \
   --session \
   --dest io.voicestream.SpeedOfSound \
   --object-path /io/voicestream/SpeedOfSound \
   --method org.gtk.Actions.Activate "trigger" [] {}
```

If you installed Nexiant Voice as a Flatpak or a Snap, D-Bus activation will automatically launch the application
